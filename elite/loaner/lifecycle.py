"""Governed Service Loaner lifecycle transitions.
...
def governed_transition(gov, store, *, principal, capability, scope, unit_id, expected_version,
                        to_state, action, effect=None, guard=None, field_updates=None,
                        correlation_id=None, idempotency_key=None):
    def business(conn):
        cur = store.get_unit(unit_id)
        if cur is None:
            raise ValidationError(technical_detail="service loaner unit not found")
        if to_state not in TRANSITIONS.get(cur.membership_state, set()):
            raise ValidationError(message="That Service Loaner change is not allowed.",
        ...
        c = conn.execute(f"UPDATE service_loaner_unit SET {cols},version=version+1 WHERE id=? AND version=?", params)
        if c.rowcount == 0:
            raise ConcurrencyError(technical_detail=f"service loaner unit {unit_id} stale")
        eff = effect(conn, cur) if effect else {}
        store.insert_membership_history(conn, unit_id, cur.membership_state, to_state, actor=principal,
                                        action=action, reconciliation_ref=eff.get("reconciliation_ref"),
    ...
    res = gov.perform(principal_id=principal, capability=capability, scope=scope, action=action,
                      business_fn=business, target_ref=unit_id, correlation_id=correlation_id,
