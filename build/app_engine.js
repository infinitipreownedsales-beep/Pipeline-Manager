...
function computeArrivalWindows(inv, today, s){
  ...
  return out; }
// DEMAND-availability window (path-independent): when we plan for inventory to be
// available, where the target + shortage are measured. No mode, no order_lead_pad
// (that pad is a SUPPLY order->production lead). Mirror of resolve_demand_window.
function resolveDemandWindow(inv, today, s){ let auto=computeArrivalWindows(inv,today,s), out={};
  ...
  return out; }
// DEPRECATED (3a): mode-resolved supply window. `mode` is now a no-op for demand;
// acquisition timing lives in supplyFeasibility. Kept for backward compat only.
...
// Projection at the DEMAND-availability window — path-independent (the old
// MID-MONTH 'right-now snapshot' is gone; immediate acquisition is now a Supply
// workflow, not a different demand calc). Mirror of project_at_arrival.
...
function buildLines(s,metrics,seas,positions,agedBrakes,overrideMap,windows,demoReturns){
  ...
  effectiveRoster(s).forEach((c,i)=>{
    ...
    let win=windows[model];   // path-independent DEMAND-availability window (no mode, no pad)
...
// ---- Attribute-premium framework (Step 5) — the single, extensible mechanism
// _retailAt uses to EXPLAIN projected resale:
//   Projected Resale = Base Value + Exterior Premium + Interior Premium
//                      + Future Attribute Premiums
// Each attribute is computed independently, capped, sample-counted and confidence-
// scored — nothing is a hidden blended adjustment. Add a new factor (package, wheel
// package, drivetrain, ...) with ONE entry in _ATTR_PREMIUMS; it plugs into _retailAt
// with no further refactor. _retailAt stays the ONLY valuation engine — this is a
// decomposition of it, not a second path.
function _canonInt(v){ return (v==null||String(v).trim()==="")?null:String(v).trim().toUpperCase(); }
var _ATTR_PREMIUMS=[
  // Exterior: inventory ext CODE -> group via the user color_map; comps already carry
  // the history ext NAME mapped to the same group (c.color). Unchanged behavior.
  {name:"exterior", target:function(ui,s){ return (s.color_map||{})[String(ui.ext||"").toUpperCase()]||null; }, compVal:function(c){ return c.color; }},
  // Interior: FULLY WIRED but data-limited today — history Interior Color is blank for
  // INFINITI QX, and inventory stores an interior CODE while history stores a NAME, so
  // values won't match -> $0 (byte-identical). A DATA gap, not an engine gap.
  // >>> EXTENSION POINT <<< when a reliable interior code->canonical-group map exists,
  // replace the target below with that lookup (mirroring color_map) — nothing else.
  {name:"interior", target:function(ui,s){ return _canonInt(ui.int); }, compVal:function(c){ return _canonInt(c.int); }},
];
// One attribute's premium: capped median price delta of comps matching the unit's
// value vs the rest, with sample count + confidence. The exact math the exterior
// premium always used, now reusable. Sparse/absent -> {premium:0} (unchanged).
function _attrPremium(comps, base, dts, compVal, target, cap){
  if(target==null||target==="") return {premium:0, n:0, confidence:0, dtsDelta:null};
  let same=comps.filter(function(c){ return compVal(c)===target; });
  if(same.length<2 || (comps.length-same.length)<1) return {premium:0, n:same.length, confidence:0, dtsDelta:null};
  let sameMed=_wmedComps(same,"price"), sameDts=_wmedComps(same,"dts");
  if(sameMed==null||base==null) return {premium:0, n:same.length, confidence:0, dtsDelta:null};
  let prem=Math.round(Math.max(-cap,Math.min(cap,sameMed-base)));
  let dd=(sameDts!=null&&dts!=null)?Math.round(sameDts-dts):null;
  // confidence 0..1 by sample size (same curve as loaner_engine's confidence();
  // computed inline so it's independent of module scope).
  let conf=Math.round(Math.min(1,Math.log(same.length+1)/Math.log(31))*1000)/1000;
  return {premium:prem, n:same.length, confidence:conf, dtsDelta:dd};
}
function _retailAt(res, ui, months){
  ...
  if(D&&D.active&&D.a&&(D.a.compsRange||D.a.getComps)){
    ...
    let comps=raw.map(c=>{ let r=_interpRet(curve, c.age), adj=c.price;
      ...
      return {price:adj, raw:c.price, dts:c.dts, age:c.age, color:c.color, ext:c.ext, int:c.int, year:c.year, trim:c.trim, date:c.date, weight:c.weight}; });
    if(comps.length){
      ...
      // Attribute premiums (exterior + interior + future) — each independently
      // computed and explainable; base + Σ premiums = projected resale.
      let cap=0.15*(base||0), attrs=[];
      _ATTR_PREMIUMS.forEach(function(a){ let tv=a.target(ui,s);
        let r=_attrPremium(comps, base, dts, a.compVal, tv, cap);
        attrs.push({attr:a.name, value:tv, premium:r.premium, n:r.n, confidence:r.confidence, dtsDelta:r.dtsDelta}); });
      // Additive under ONE combined guard so ext+int(+future) can't over-swing. If
      // clipped, scale proportionally so the breakdown still sums to the applied
      // total (base + Σ premiums = final is always exact — never a hidden blend).
      let rawSum=attrs.reduce(function(t,a){ return t+a.premium; },0);
      let premSum=Math.max(-cap,Math.min(cap,rawSum));
      if(rawSum!==0 && premSum!==rawSum){ let scl=premSum/rawSum;
        attrs.forEach(function(a){ a.premium=Math.round(a.premium*scl); });
        premSum=attrs.reduce(function(t,a){ return t+a.premium; },0); }
      let extA=attrs[0]||{}, intA=attrs[1]||{};
      let aged=Math.round(base*ageFactor);
      let lo=_wpctComps(comps,"price",0.25), hi=_wpctComps(comps,"price",0.75);
      return {price:aged+premSum, base:aged, n:comps.length, dts:(dts!=null?Math.round(dts):null),
        low:(lo!=null?Math.round(lo*ageFactor)+premSum:null), high:(hi!=null?Math.round(hi*ageFactor)+premSum:null),
        month:saleMonthIdx, premiums:attrs, premiumTotal:premSum,
        color:extA.value||null, colorPrem:extA.premium||0, colorN:extA.n||0, colorDtsDelta:(extA.dtsDelta!=null?extA.dtsDelta:null),
        intColor:intA.value||null, intPrem:intA.premium||0, intN:intA.n||0, intConfidence:(intA.confidence!=null?intA.confidence:0),
...
function _retireTiming(res, unit, costFn, policy){
  ...
  let ui={model:unit.model, trim:info.trim||"", ext:unit.ext||"", int:unit.int||"", year:unit.year||res.tb.today.getFullYear(), cost:info.cost, msrp:info.msrp||info.cost,
...
function loanerFleet(res){ let s=res.settings, today=res.tb.today, rows=[], releasing=0;
  ...
  (s.loaner_units||[]).forEach(e=>{ let stock=String(e.stock||"").trim(); if(!stock) return;
    ...
    let rt=_retireTiming(res, {model:model,code:code,ext:ext,int:(u?u.int:(e.int||"")),miles:miles,months:months,year:uyear,entryMonth:entryMonth,entryYear:uyear}, costFn, policy);
...
function runEngine(inv,sales,s,today){
  ...
  let demandWindows=resolveDemandWindow(inv,today,s);   // path-independent demand window
  ...
  let lines=buildLines(s,metrics,seas,positions,agedBrakes,overrideMap,demandWindows,demoReturns);
  let orphans=findOrphans(sales,effectiveRoster(s));
  let res={settings:s,tb:tb,metrics:metrics,seas:seas,positions:positions,lines:lines,demoUnits:demoUnits,sales:sales,orphans:orphans,inv:inv,invCount:inv.length,salesCount:sales.length,windows:demandWindows};
